<?php
	...
	class Model_siswa extends CI_Model
	{
		...
		public function getNilai($nim){
			...
			$data = $this->db
					->select("d.nama,
								...
								b.kd_ruangan,
								a.id_jadwal")
					->join('tbl_jadwal b','a.id_jadwal=b.id_jadwal')
					->join('tbl_mapel c','b.kd_mapel=c.kd_mapel')
					->join('tbl_siswa d','a.nim=d.nim')
					->join('tbl_tahun_akademik e','b.id_tahun_akademik=e.id_tahun_akademik')
					->where('e.is_aktif','Y')
					->where('b.semester','e.semester',FALSE)
					->where('a.nim',$nim)
					->get('tbl_nilai a')
					->result_array();
			return $data;
		...
		public function getData(){
			$data = $this->db->get('tbl_siswa')->row_array();
			return $data;
		}
